...
class GFFondy extends GFPaymentAddOn
{
    ...
    protected $_title = 'Gravity Forms Alliance bank Add-On';
    protected $_short_title = 'Alliance bank';
    protected $_supports_callbacks = true;
    private $production_url = 'https://payment.albpay.io/api/checkout/url/';
    private $sandbox_url = 'https://payment.albpay.io/api/checkout/url/';
    ...
    public function plugin_settings_fields()
    {
        $description = '
            ...
            esc_html__('Gravity Forms requires to be create and enabled your Alliance bank account. Follow the following steps to confirm it is enabled.', 'gravityformsfondy') .
            '</p>
			<ul>
			<li>' . sprintf(esc_html__('Create you free %sAlliance bank account.%s', 'gravityformsfondy'), '<a href="https://portal.albpay.io/mportal/#/account/registration" target="_blank">', '</a>') . '</li>' .
            ...
            array(
                ...
                'fields' => array(
                    array(
                        ...
                        'label' => esc_html__('Alliance bank setting', 'gravityformsfondy'),
                        'type' => 'checkbox',
                        'choices' => array(array('label' => esc_html__('Confirm that you have configured your Alliance bank account in Merchant portal.', 'gravityformsfondy'), 'name' => 'gf_fondy_configured'))
                    ),
                    array(
                        'type' => 'save',
                        'messages' => array(
    ...
    public function feed_list_no_item_message()
    {
        ...
        if (!rgar($settings, 'gf_fondy_configured')) {
            return sprintf(esc_html__('To get started, please configure your %sAlliance bank Settings%s!', 'gravityformsfondy'), '<a href="' . admin_url('admin.php?page=gf_settings&subview=' . $this->_slug) . '">', '</a>');
        } else {
    ...
    public function feed_settings_fields()
    {
        ...
        $fields = array(
            array(
                'name' => 'MerchantID',
                'label' => esc_html__('Alliance bank Merchant ID', 'gravityformsfondy'),
                'type' => 'text',
                'class' => 'medium',
                'required' => true,
            ...
            array(
                'name' => 'SecretKey',
                'label' => esc_html__('Alliance bank secret key ', 'gravityformsfondy'),
                'type' => 'text',
                'class' => 'medium',
                'required' => true,
                'tooltip' => '<h6>' . esc_html__('Alliance bank secret key from portal', 'gravityformsfondy') . '</h6>'
            ),
            array(
                'name' => 'mode',
                ...
                'tooltip' => '<h6>' . esc_html__('Mode', 'gravityformsfondy') . '</h6>' . esc_html__('Select Production to receive live payments. Select Test for testing purposes when using the Alliance bank development sandbox.', 'gravityformsfondy')
            ),
        );
    ...
    {
        return esc_html__('Alliance bank Field', 'gravityformsfondy');
    }
    ...
    public function redirect_url($feed, $submission_data, $form, $entry)
    {
        ...
        if (!$data) {
            $this->log_debug(__METHOD__ . '(): NOT sending to Alliance bank: The price is either zero.');
            return '';
        }
        ...
        $this->log_debug(__METHOD__ . "(): Sending to Alliance bank: " . json_encode($data));
    ...
    public function callback()
    {
        ...
        $this->log_debug(__METHOD__ . '(): Alliance bank request received. Starting to process => ' . print_r($_POST, true));
        ...
        if (is_wp_error($is_verified)) {
            $this->log_error(__METHOD__ . '(): Alliance bank verification failed with an error. Aborting with a 500 error so that FODNY is resent.');
            return new WP_Error('FondyVerificationError', 'There was an error when verifying the Alliance bank message with', array('status_header' => 500));
        } elseif (!$is_verified) {
            $this->log_error(__METHOD__ . '(): Alliance bank request could not be verified. Aborting.');
            return false;
        }

        $this->log_debug(__METHOD__ . '(): Alliance bank message successfully verified');
    ...
    private function get_reason($code)
    {
        ...
        switch (strtolower($code)) {
            ...
            case 'declined':
                return esc_html__('Order is declined by Alliance bank payment gateway or by bank or by external payment system', 'gravityformsfondy');
            case 'approved':
            ...
            default:
                return empty($code) ? esc_html__('Reason has not been specified. For more information, contact Alliance bank Customer Service.', 'gravityformsfondy') : $code;
        }
    }
    ...
    public function temporary_plugin_page()
    {
        ...
        <div class="wrap about-wrap">
            <h1><?php _e('Alliance bank Add-On v1.0', 'gravityformsfondy') ?></h1>
            <div
                class="about-text"><?php esc_html_e('Thank you! Alliance bank simple integration.', 'gravityformsfondy') ?></div>
            <div class="changelog">
                <hr/>
                <div class="feature-section col two-col">
                    <div class="col-1">
                        <p><?php esc_html_e('Alliance bank Feeds are now accessed via the Alliance bank sub-menu within the Form Settings for the Form you would like to integrate Alliance bank with.', 'gravityformsfondy') ?></p>
                    </div>
