...
class RCP_Payment_Gateway_Fondy extends RCP_Payment_Gateway
{
    ...
    public function init()
    {
        ...
        $this->supports[] = 'fees';
        $this->supports[] = 'trial';
        $this->api_endpoint = 'https://payment.albpay.io/api/checkout/url/';
    ...
    public function process_webhooks()
    {
        ...
        rcp_log('Starting to process Alliance bank webhook.');
        ...
        if ($paymentInfo === true) {
            ...
            if (!$member || !$member->ID > 0) {
                rcp_log('Alliance bank - member ID not found.');
            ...
            $payment_data = array(
                ...
                'payment_type' => 'Alliance bank Credit Card',
                ...
                'user_id' => $user_id,
                'gateway' => 'Alliance bank',
                'transaction_id' => $posted['payment_id'] ?? '',
                'status' => 'complete'
            );
            $rcp_payments = new RCP_Payments();

            rcp_log(sprintf('Processing Alliance bank. Payment status: %s', $posted['order_status']));
            ...
            switch (strtolower($posted['order_status'])) :
                case 'approved':
                    ...
                    break;
                case 'declined' :
                    rcp_log('Processing Alliance bank declined webhook.');
                    $member->cancel();
                    die('payment declined');
                case 'expired' :
                    rcp_log('Processing Alliance bank expired webhook.');

                    $member->set_status('expired');

                    $member->set_expiration_date(date('Y-m-d H:i:s', strtotime($posted['order_time'])));

                    $member->add_note(__('Subscription expired in Alliance bank', 'fondy_rcp'));
