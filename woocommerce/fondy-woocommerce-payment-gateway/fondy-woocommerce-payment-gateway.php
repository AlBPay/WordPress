 ...
 * Plugin URI: https://example.com/gb/plugins/woocommerce/
...
if ( ! class_exists( 'WC_Fondy' ) ) {
    class WC_Fondy
    {
        ...
        public function woocommerce_fondy_wc_not_supported_notice()
        {
            ...
            $message = sprintf(__('Payment Gateway Alliance bank requires WooCommerce %1$s or greater to be installed and active. WooCommerce %2$s is no longer supported.', 'fondy-woocommerce-payment-gateway'), WC_FONDY_MIN_WC_VER, WC_VERSION);
        ...
        public function woocommerce_fondy_php_not_supported_notice()
        {
            ...
            $message = sprintf(__('The minimum PHP version required for Alliance bank Payment Gateway is %1$s. You are running %2$s.', 'fondy-woocommerce-payment-gateway'), WC_FONDY_MIN_PHP_VER, phpversion());
            echo '<div class="notice notice-error is-dismissible"> <p>' . $message . '</p></div>';
        }
