...
//            $renewal_order->add_order_note('Order amount is: ' . $amount_to_charge);
            ...
            if ($subscriptionPayment->order_status === 'approved') {
                ...
                $renewal_order->add_order_note("Fondy subscription payment successful.<br/>Alliance bank ID: $subscriptionPayment->payment_id");
            } else {
                throw new Exception("Transaction ERROR: order $subscriptionPayment->order_status<br/>Alliance bank ID: $subscriptionPayment->payment_id");
            }
        } catch (Exception $e) {
